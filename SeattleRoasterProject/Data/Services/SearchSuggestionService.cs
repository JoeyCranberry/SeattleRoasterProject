...
{
    public async Task<List<SearchSuggestion>> BuildSuggestions(List<RoasterModel> roasters, List<BeanModel> beans,
        List<TastingNoteModel> allTastingNotes)
    {
        ...
        {
            if (beans != null && beans.Where(b => b.ProcessingMethods != null && b.ProcessingMethods.Contains(method))
                    .Any())
                suggestions.Add(
            ...
            foreach (var bean in beans)
            {
                if (bean.Origins != null)
                {
                    foreach (var origin in bean.Origins)
                    {
                        ...
                        {
                            var fullRegion = origin.Region + ", " + BeanOrigin.GetCountryDisplayName(origin.Country);
    ...
    public List<SearchSuggestion> GetSuggestionMatches(List<SearchSuggestion> allSuggestions, string search,
        List<SearchSuggestion> acceptedSuggestions, int maxSuggestions = 10)
    {
        List<SearchSuggestion> matchingSuggestions = new();

        var inputWordSplit = search.Split(' ').ToList();

        // Pupled Natural San Pedro, Guatemala
        // 0	  1		  2   3		 4
        for (var i = inputWordSplit.Count - 1; i >= 0; i--)
        {
            var largestSearchTerm = "";
            for (var j = i; j < inputWordSplit.Count; j++)
            {
            ...
            var exactMatches =
                GetSuggestionExactMatches(allSuggestions, largestSearchTerm, acceptedSuggestions, maxSuggestions);
        ...
        {
            for (var i = inputWordSplit.Count; i > 0; i--)
            {
                var largestSearchTerm = "";
                for (var j = i; j < inputWordSplit.Count; j++)
                {
                ...
                var exactMatches = GetSuggestionApproximateMatches(allSuggestions, largestSearchTerm,
                    acceptedSuggestions, maxSuggestions);
            ...
            switch (suggestion.SuggestionCategory)
            {
                ...
                case SearchSuggestion.SuggestionType.Region:
                    if (suggestion.MatchingIntValue != null)
                    {
                        ...
                        var commaIndex = suggestion.DisplayName.IndexOf(',');
                        if (commaIndex != -1)
                        {
                            var region = suggestion.DisplayName.Substring(0, commaIndex);
    ...
    private List<SearchSuggestion> GetSuggestionExactMatches(List<SearchSuggestion> allSuggestions, string search,
        List<SearchSuggestion> acceptedSuggestions, int maxSuggestions = 10)
    {
        List<SearchSuggestion> matchingSuggestions = new();
        ...
        foreach (var suggestion in allSuggestions)
        {
            ...
            if (!acceptedSuggestions.Where(vs =>
                        vs.DisplayName == suggestion.DisplayName &&
                        vs.SuggestionCategory == suggestion.SuggestionCategory)
                    .Any())
            {
                foreach (var term in suggestion.MatchingStrings)
                {
    ...
    private List<SearchSuggestion> GetSuggestionApproximateMatches(List<SearchSuggestion> allSuggestions, string search,
        List<SearchSuggestion> acceptedSuggestions, int maxSuggestions = 10)
    {
        List<SearchSuggestion> matchingSuggestions = new();
    ...
    {
        var searchSuggestions = allTastingNotes.Select(note => new SearchSuggestion(note.NoteName,
            SearchSuggestion.SuggestionType.Tasting_Note, note.Aliases ?? new List<string>(), null, null));
...
public class SearchSuggestion
{
    ...
    public SearchSuggestion(string displayName, SuggestionType category, int? matchingIntValue) : this(displayName,
        category, new List<string> { displayName }, matchingIntValue, null)
    {
    }

    public SearchSuggestion(string displayName, SuggestionType category) : this(displayName, category,
        new List<string> { displayName }, null, null)
    {
    }

    public SearchSuggestion(string displayName, SuggestionType category, string? matchingIdValue) : this(displayName,
        category, new List<string> { displayName }, null, matchingIdValue)
    {
    }

    public SearchSuggestion(string displayName, SuggestionType category, List<string> additionalMatchingStrings,
        int? matchingIntValue, string? matchingIdValue)
    {
        DisplayName = displayName;
        MatchingStrings = new List<string> { displayName };
